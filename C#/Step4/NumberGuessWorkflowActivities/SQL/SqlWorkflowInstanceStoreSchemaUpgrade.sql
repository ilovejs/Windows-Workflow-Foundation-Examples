...
if not exists (select 1 from sys.objects where object_id = object_id(N'[System.Activities.DurableInstancing].[DefinitionIdentityTable]') and type in (N'U'))
 begin
...
if not exists (select 1 from sys.indexes where name = 'CIX_DefinitionIdentityTable' and object_id = object_id('[System.Activities.DurableInstancing].[DefinitionIdentityTable]'))
 begin
	if (serverproperty('EngineEdition') = 5)
	 begin
		exec('
			create unique clustered index [CIX_DefinitionIdentityTable]
				on [System.Activities.DurableInstancing].[DefinitionIdentityTable] ([SurrogateIdentityId])
		')
	 end
	else
	 begin
...
if not exists (select 1 from sys.indexes where name = 'NCIX_DefinitionIdentityTable_DefinitionIdentityHash' and object_id = object_id('[System.Activities.DurableInstancing].[DefinitionIdentityTable]'))
 begin
	if (serverproperty('EngineEdition') = 5)
	 begin
		exec('
...
if not exists (select 1 from sys.indexes where name = 'NCIX_DefinitionIdentityTable_DefinitionIdentityAnyRevisionHash' and object_id = object_id('[System.Activities.DurableInstancing].[DefinitionIdentityTable]'))
 begin
	if (serverproperty('EngineEdition') = 5)
	 begin
		exec('
...
if not exists (select 1 from sys.objects where object_id = object_id(N'[System.Activities.DurableInstancing].[IdentityOwnerTable]') and type in (N'U'))
 begin
...
if not exists (select 1 from sys.indexes where name = 'NCIX_IdentityOwnerTable_IdentityOwner' and object_id = object_id('[System.Activities.DurableInstancing].[IdentityOwnerTable]'))
 begin
	if (serverproperty('EngineEdition') = 5)
	 begin
		exec('
			create unique clustered index [NCIX_IdentityOwnerTable_IdentityOwner]
				on [System.Activities.DurableInstancing].[IdentityOwnerTable] ([SurrogateLockOwnerId], [SurrogateIdentityId])
		')
	 end
	else
	 begin
...
if not exists (select 1 from [System.Activities.DurableInstancing].[IdentityOwnerTable])
 begin
	...
	insert	[System.Activities.DurableInstancing].[IdentityOwnerTable]
	(
			[SurrogateIdentityId],
	...
	select	@surrogateIdentityId,
			[SurrogateLockOwnerId]
	from	[System.Activities.DurableInstancing].[LockOwnersTable]
 end
go
...
if not exists (	select		1
				from		sys.columns	
				where		object_id = object_id('[System.Activities.DurableInstancing].[InstancesTable]')
				and			name = 'SurrogateIdentityId')
 begin
	alter table [System.Activities.DurableInstancing].[InstancesTable]
		add [SurrogateIdentityId] bigint null
 end
go
...
if exists (	select		1
 ...
 begin		
	...
	update	[System.Activities.DurableInstancing].[InstancesTable]
	set		[SurrogateIdentityId] = @surrogateIdentityId
	where	[SurrogateIdentityId] is null
	
	alter table [System.Activities.DurableInstancing].[InstancesTable]
		alter column [SurrogateIdentityId] bigint not null
 end
go
...
if not exists (	select		1
				from		sys.columns	
				where		object_id = object_id('[System.Activities.DurableInstancing].[RunnableInstancesTable]')
				and			name = 'SurrogateIdentityId')
 begin
...
if exists (	select		1
 ...
 begin						
	...
	update	[System.Activities.DurableInstancing].[RunnableInstancesTable]
	set		[SurrogateIdentityId] = @surrogateIdentityId
...
if not exists (select 1 from sys.indexes where name = 'NCIX_RunnableInstancesTable_SurrogateIdentityId' and object_id = object_id('[System.Activities.DurableInstancing].[RunnableInstancesTable]'))
 begin
	if (serverproperty('EngineEdition') = 5)
	 begin
		exec('
...
if not exists (	select		1
				from		sys.columns
				where		object_id = object_id('[System.Activities.DurableInstancing].[LockOwnersTable]')
				and			name = 'WorkflowIdentityFilter')
 begin
	alter table [System.Activities.DurableInstancing].[LockOwnersTable]
		add [WorkflowIdentityFilter] tinyint null
 end
go
...
if exists (	select		1
			...
			and			is_nullable = 1)
 begin
	update	[System.Activities.DurableInstancing].[LockOwnersTable]
	set		[WorkflowIdentityFilter] = 0
	
	alter table [System.Activities.DurableInstancing].[LockOwnersTable]
		alter column [WorkflowIdentityFilter] tinyint not null
 end
go

if not exists (select 1 from sys.indexes where name = 'NCIX_LockOwnersTable_WorkflowIdentityFilter' and object_id = object_id('[System.Activities.DurableInstancing].[LockOwnersTable]'))
 begin
	if (serverproperty('EngineEdition') = 5)
	begin
		exec('
...
if exists (select 1 from sys.views where object_id = object_id(N'[System.Activities.DurableInstancing].[Instances]'))
